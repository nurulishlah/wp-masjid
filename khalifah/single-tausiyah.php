		        	...
		        	while (have_posts()): the_post();
			     	?>
						...
						<div class="sing__meta">
					        <strong><?php echo __( 'By', 'wp-masjid' ); ?></strong> : <?php the_author(); ?> <span>
							    <?php
								    $kat_tau = get_the_terms( $post->ID , 'kat-tausiyah' );
									if ( $kat_tau != null ) {
										foreach( $kat_tau as $termjab ) {
										    $term_link = get_term_link( $termjab, 'kat-tausiyah');
										    echo '<a href="' . esc_url( $term_link ) . '">' . esc_html( $termjab->name ) . '</a> ';
										    unset($termjab); 
									    } 
									}
								?>
						...
						<div class="sing__content">
						    <?php 
						    	the_content();
								$top_tau = get_the_terms( $post->ID , 'topik' );
									if ( $top_tau != null ) {
										echo '<div class="sing__tag div__clear">';
										foreach( $top_tau as $termjab ) {
										    $term_link = get_term_link( $termjab, 'topik');
										    echo '<a href="' . esc_url( $term_link ) . '">' . esc_html( $termjab->name ) . '</a> ';
										    unset($termjab); 
									    } 
										echo '</div>';
